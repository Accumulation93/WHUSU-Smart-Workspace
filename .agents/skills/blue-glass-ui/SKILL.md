...
- The top navigation must use the shared safe-area-aware navigation component. Measure the status bar with `wx.getWindowInfo()` and the capsule with `wx.getMenuButtonBoundingClientRect()`, then center the title inside its own navigation row instead of treating the status bar and title row as one undivided white block. This prevents Huawei and HarmonyOS devices with an uninterrupted light status area from making the title look pinned to the bottom. Do not restore per-page native navigation or hard-code a device model.
