...
- Treat authentication governance as part of the HR member directory. Keep the outer member card and detail summary concise with one merged account-state label. Do not repeat that state as “current status”, “WeChat binding”, or an abstract “recovery method” fact card in the account section; show only actionable verification codes, recovery codes, devices, pending recovery, and their controls. Place individual verification, recovery, freeze and global-unbind actions in the member detail dialog. Keep batch selection and credential actions visibly aligned with the existing filters, without a separate framed module; use a deterministic two-column phone grid and four-column Pad grid, and use native disabled states when no visible member is applicable. Only “认证设置” may remain as a subordinate HR tab. Never create a second authentication people directory or expose internal operation logs.
