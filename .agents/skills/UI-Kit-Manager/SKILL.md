...
- “离开当前组织”停用成员关系和岗位并撤销该组织普通管理员授权、权限覆盖和管理员上下文会话，但不删除自然人、全局账号或历史，也不修改全局超级管理员治理权。永久删除必须先做全引用预检，业务记录阻断、允许的未执行引用事务清理、零候选规则停用；组织删除不得影响其他组织，彻底删除自然人仅限超级管理员且只保留去标识审计。冻结、解绑、重置全局账号仅限 `auth.accounts.global_manage`。
