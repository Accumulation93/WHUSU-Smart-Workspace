...
- 人事目录一人一卡，岗位在详情中分组，无岗位显示中性气泡。资料审核状态与完整度分开；生效资料维护和待审资料审核分权，驳回原因必填。
- “离开当前组织”只停用成员关系和岗位，不删除自然人、全局账号或历史。冻结、解绑、重置全局账号仅限 `auth.accounts.global_manage`。
- 部门、身份类别、职能组被任何岗位、规则、快照或历史引用后禁止删除，并返回引用分类和数量；不得自动替换或通过停用规避完整性。
- 人事比较卡、标题、筛选和操作必须有语义玻璃容器。长弹窗采用固定标题、直接子级滚动正文和固定操作栏，内容驱动高度并受视口安全上限约束，三档设备都必须能滚动到底。
