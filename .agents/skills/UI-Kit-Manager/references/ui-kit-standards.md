...
场地借用管理端的“待我审批”必须作为顶部管理页签独立呈现，不得继续嵌在“借用管理”的筛选卡中。待我审批页内的“审批历史”入口使用紧凑链接式控件；历史列表每次从当前组织全部借用记录实时匹配当前身份和当前操作者的审批快照或旧审批字段，不建立单独历史表，并明确显示处理结果、时间和意见。旧快照缺少身份上下文时，按稳定人员/管理员身份字段兼容匹配，不能因数据格式升级显示为空。
