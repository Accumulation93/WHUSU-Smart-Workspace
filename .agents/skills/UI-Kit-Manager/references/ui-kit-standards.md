...
3. 一名自然人在成员列表只出现一张卡；岗位只在成员详情分组展示。无岗位在职成员仍可见，使用中性状态气泡，不显示虚构的默认部门、身份类别或职能组。
...
6. “离开当前组织”与删除自然人、删除账号严格分离。账号高危操作只对具备 `auth.accounts.global_manage` 的用户显示；其他组织管理员只显示合并账号状态和联系全局管理员提示。
7. 部门、身份类别、职能组一旦被任何岗位、规则、快照或历史引用，删除入口必须被阻止并展示引用分类/数量；不得提供自动替换、级联删除或停用绕过。
8. 人事标题、筛选、批量操作和岗位组至少使用一层语义玻璃容器。资料对照卡先显示字段名，再并列/分栏展示“当前生效”和“待审核”；手机可纵向，Pad 可双列，但信息顺序一致。
