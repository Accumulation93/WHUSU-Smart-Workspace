...
6. “离开当前组织”、永久删除当前组织成员记录、彻底删除自然人和删除账号严格分离。离任同步撤销当前组织普通管理员授权、权限覆盖和管理员上下文会话，但不得修改全局超级管理员治理权。永久删除必须先做全引用预检：业务事实作为阻断项，允许清理的未执行引用作为自动清理项；清理后零候选的模板、步骤或规则必须停用并返回待修复清单，不得扩大候选。组织删除不得影响其他组织或全局账号；彻底删除仅限超级管理员并要求输入学号二次确认。账号高危操作只对具备 `auth.accounts.global_manage` 的用户显示；其他组织管理员只显示合并账号状态和联系全局管理员提示。
