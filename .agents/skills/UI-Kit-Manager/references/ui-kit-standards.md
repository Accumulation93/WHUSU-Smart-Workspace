...
普通用户端必须提供可见的“审批历史”入口，审批历史详情、日程点击详情和我的借用点击详情必须复用同一个借用详情组件与数据形状。服务端必须先完成当前组织、当前身份和当前操作者的历史权限匹配，再返回详情。详情进度必须同时携带全部 `flowSteps` 与 `snapshots`，显示已完成步数取数据库当前步骤与快照最高已完成步骤的并集并限制在总步骤内，确保后续步骤完成后不会停留在旧进度。
