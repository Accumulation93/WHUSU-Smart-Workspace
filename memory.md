...
- **登录口令最低长度放开（2026-08-05）**：口令最短长度默认改为 0（不限制），客户端、服务端、管理端策略输入框及 `auth_policy` 默认值全部移除 `|| 12` 兜底；管理端“口令最短长度”输入框清空时按 0 处理，不再跳回 12。输入阶段不做任何校验，仅保存时校验：口令为空提示“请输入登录口令”，管理员显式配置了最短长度且不满足时才提示“口令长度不足”。新增幂等迁移将现有默认策略行的 `passphrase_min_length` 置 0。
