...
- **认证功能归属人事（2026-08-04）**：管理端不再设置“认证与账号”人员目录。人员认证、账号状态、恢复码、冻结与恢复操作直接显示在“人事信息 → 成员资料”的现有成员卡片中，批量操作复用成员资料的筛选结果与选择状态；只有“认证设置”保留独立页签。普通用户的恢复码、登录口令和设备管理仍统一并入“人事信息 → 账号与登录”。旧地址仅负责跳转兼容，内部操作日志只保留在服务端审计中。
...
- 管理端认证、账号状态、恢复方式和认证设置全部归入人事信息；前端不再展示独立认证页或操作记录，服务端继续保留安全审计。
