...
if [ "$ROOT_ADMIN_COUNT" -ge 1 ] 2>/dev/null; then
  ...
  if [ "$CREATE_ADMIN" = "y" ] || [ "$CREATE_ADMIN" = "Y" ] || [ "$CREATE_ADMIN" = "yes" ] || [ "$CREATE_ADMIN" = "YES" ]; then
    ...
    if $MYSQL_CMD "$DB_NAME" -e "INSERT INTO admin_info (id, name, student_id, admin_level, bind_status, invite_code, invited_at) VALUES (REPLACE(UUID(), '-', ''), '$ESC_NAME', '$ESC_STUDENT', 'root_admin', 'invited', '$ESC_INVITE', UTC_TIMESTAMP());" 2>&1; then
