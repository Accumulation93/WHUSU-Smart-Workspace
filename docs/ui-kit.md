...
- 人事目录坚持“一名自然人一张成员卡”。卡片显示组织成员状态和合并账号状态；多个岗位只在成员详情中分组，不得把同一人按岗位复制成多张卡。无岗位在职成员显示中性状态气泡。
...
- 标题、筛选、批量操作、岗位组和资料比较区至少拥有一层语义玻璃表面。长弹窗采用固定标题、直接子级滚动正文、固定底部操作区；高度内容驱动且受视口安全上限约束，手机、Pad 竖屏、Pad 横屏都必须能滚动到底并操作。
