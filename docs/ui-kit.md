...
- 门户和消息中心的业务类别与身份组成明确上下文行，组织名称和“当前”独占下一整行。组织名称保持完整，空间不足时只在该语义行内按词自然换行、不得拆字；“当前”固定不收缩。
...
- 审核提交/编辑页的模板步骤预览中，第一步若允许指定审批人，必须在第一步步骤卡内部提供“指定/修改指定”紧凑入口；已指定人员直接以气泡显示在同一张步骤卡内，可在气泡内移除。禁止再渲染独立的“第一步审批人”卡片；第一步不允许指定时不显示该入口。
