...
- 工作上下文选择器展示组织和岗位标签；“身份类别”只作为岗位维度，不得替代“岗位”或“工作上下文”。前端状态和交互统一使用 `activeContextId`。
...
- 评分页的首次评分和已评分修订都可编辑：点击题目必须在一次状态更新中同步当前题、快捷分值和键盘展开态；完整历史快照显示“已评分 · 可修改”，提交时先归档旧版本再保存当前修订。只有缺少可验证题目范围的早期记录才显示只读及原因。题目使用“题号气泡 + 正文标题 + 最高分气泡”，模板卡底部只保留“小计 + 分值”；标题、小计、总分均使用正文/强调/数值语义字号，禁止使用页面标题字号造成手机或 Pad 碎行。
