...
- 普通用户端同样必须提供“审批历史”入口。审批历史详情、日程点击详情和我的借用点击详情必须复用同一个借用详情组件与数据形状；服务端先完成当前组织、当前身份和当前操作者的历史权限匹配，再返回可展示详情。审批进度必须同时携带全部 `flowSteps` 与 `snapshots`，显示已完成步数取数据库当前步骤和快照最高已完成步骤的并集并限制在总步骤内，确保后续步骤完成后不会停留在旧进度。
