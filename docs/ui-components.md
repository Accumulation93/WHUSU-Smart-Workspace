...
- 人员/选项选择列表必须能翻到底并完整显示最后一项：内层 `scroll-view` 使用 `box-sizing: border-box` 并在内容底部预留足够留白（含安全区），禁止固定高度把最后一项裁掉；嵌套滚动保持 `nested-scroll-enabled`，手机与 Pad 都要实测“翻到底可见最后一项”。候选列表与已选列表同样适用。
