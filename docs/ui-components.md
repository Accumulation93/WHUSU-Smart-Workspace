...
- 认证与账号复用人事管理的搜索行、人员列表、状态标签和分区卡，不得再建立灰暗、拥挤的独立视觉体系；管理端不再提供独立认证页。
...
- 管理端在人事信息内提供“成员资料 / 认证与账号 / 认证设置”三个模式，共用同一份完整人员目录、筛选行和人员卡片；操作记录只保留在服务端安全审计，不出现在用户界面。
- 认证状态、账号状态、恢复方式和已登录设备数量作为人员行的元数据展示；冻结、解冻和凭据操作只更新目标行，不刷新整页。
