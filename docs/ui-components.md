...
- 管理端在人事信息内只保留“成员资料 / 认证设置”两个模式。成员外卡保持简洁，只展示姓名、学号、岗位数量、资料状态和一个合并账号状态；单人认证、恢复、冻结与全局解绑全部放在成员详情的“账号与认证”分区。操作记录只保留在服务端安全审计，不出现在用户界面。
- 批量选择框与筛选工具长期显示；全选当前结果、反选、清空以及认证码/恢复码生成与撤销同筛选控件并列，不另绘制模块外框。无可用目标或正在处理时保留按钮并使用原生禁用态，无权限时隐藏。
