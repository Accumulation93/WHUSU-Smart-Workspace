...
- 成员外卡的批量选择同样遵循卡片左上选择控件契约：使用独立等宽紧凑胶囊“选择 / 取消”（不带 ✓），不得嵌入姓名、学号或状态行；选中转绿色，禁用态弱化。行内“查看/编辑、删除、通过、驳回”等操作使用 `.hr-action-chip`，圆角走标准按钮档（手机 28rpx、Pad 竖屏 14px、Pad 横屏 12px）。微型元数据标签（CSV/校验类型等）同样使用紧凑圆角令牌。
