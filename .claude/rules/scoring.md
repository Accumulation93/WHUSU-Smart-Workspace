...
- 已提交评分允许评分人重新打开修改。读取时必须使用当前评分记录自己的 `calculation_context_snapshot` 恢复提交时题目、范围与答案，完整快照返回可编辑状态；保存前把旧记录和旧答案写入 `score_record_revisions`，再事务化更新当前答案并递增修订号，禁止无痕覆盖或套用当前模板重算。只有旧记录缺少可验证题目范围时才返回只读降级详情。
