...
- 永久删除组织成员或自然人必须先做全引用预检，并在执行时复核版本。审核、场地、评分、资料审核历史等业务事实一律阻断；允许清理的未执行规则、授权、会话、签名模板和认证数据在事务中处理，清理后零候选的模板/规则必须停用。组织删除保留其他组织和全局账号；自然人删除仅限超级管理员，禁止删除当前操作者或最后有效超级管理员，并只保留去标识审计。所有可能新增人员业务事实或签名原图的写入必须先获取人员删除屏障，禁止与永久删除形成 TOCTOU。
