...
- 登录成功进入门户必须直接使用 `wx.reLaunch` 重建页面栈；禁止先隐藏登录页根节点，再把路由调用放进该次 `setData` 的渲染回调。微信登录能力和路由调用都必须有失败或超时兜底，任何真机分支都不得永久保持按钮加载态。
- 登录成功响应中的令牌、组织、岗位、用户资料和兼容目录必须在进入门户前同步完整提交，确保后续页面只能读取到同一批登录状态。登录临界路径禁止等待 `wx.batchSetStorage` 或 `wx.setStorage` 的完成回调；旧版鸿蒙微信基础库可能不触发这些回调并造成无限加载。写入内容必须保持有界，不得采集或校验设备信息；提交完成后立即使用 `reLaunch` 进入门户。
- API 成功回调必须先兑现请求结果，再处理时区缓存、版本提醒和岗位提示等非关键副作用；每项副作用必须单独隔离同步异常。任何非关键能力异常都不得使已经收到 HTTP 响应的登录 Promise 永久停留在 pending 状态。
- 微信登录、口令登录、认领、恢复和会话自动续期不得读取、生成或提交设备安装标识作为放行条件。设备型号等信息只能在登录完成后作为可选展示信息异步采集；采集失败、设备变化或多设备并发使用均不得阻断登录、续期或进入子应用。
- 门户及消息入口进入业务分包统一使用可信路由工具。`navigateTo` 失败或回调丢失时，必须通过独立超时依次尝试 `redirectTo` 和 `reLaunch`；恢复前先核对目标页是否已生效，恢复成功不得误报失败。禁止只处理显式失败而让已登录用户点击后停留在门户。
