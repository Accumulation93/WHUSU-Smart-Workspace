...
- 场地借用规则编辑器沿用审批子应用的步骤编辑结构：固定标题、独立滚动正文、固定底部操作栏三段分离；滚动正文必须是直接子级 `scroll-view.ui-dialog-body`，保存按钮必须位于 `ui-dialog-footer`。
- 审批步骤使用紧凑网格和标签组展示。部门、职能组、身份必须在同一条条件视觉单元内按标签换行，禁止每个条件各占一整行并制造大块空白。
- 编辑条件时隐藏步骤编辑器，不能同时渲染两个编辑器；指定条件的选择器可独占一行，普通条件使用双列布局。
